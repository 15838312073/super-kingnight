...
--在字符串中检查 mscordlib 文字开始
local function faster()
    local metadata = {}
    local allRanges = gg.getRangesList()
    local stringOffset = {} --0x18  元数据， 字符串偏移量
    local strStart = {}
    ...
    for i, v in ipairs(strStart) do
        --每个字符串表都以 mscorlib 开头.dll在全局元数据中.dat
        --所以，如果前 4 个字节是 “m（0x6D） s（0x73） c（0x63） o（0x6F）”
        if v.value==0x6F63736D then return {allRanges[i]} end
    end
    return {}
end

--发现 get_fieldOfView in Ca, A, O
local function fast()
...
local function get_metadata()
    local findingMethods = {
        [1] = fastest, --正常获取元数据
        [2] = faster, --检查字符串中的 mscordlib 文字
        [3] = fast, --发现 get_fieldOfView in Ca, A, O
    }
    local metadata = {}
...
local function Dump(class_parent)
    ...
    for i, v in ipairs(res) do
        all[#all+1] = {address=v.address - (pointerSize*3), flags=pointerType} --函数指针
        all[#all+1] = {address=v.address - (pointerSize*2), flags=pointerType} --调用函数指针或字段名称指针
        all[#all+1] = {address=v.address - (pointerSize*1), flags=pointerType} --函数名称指针或字段类型指针
        all[#all+1] = {address=v.address + pointerSize, flags=gg.TYPE_DWORD} --函数类型指针或字段偏移量
    end
...
local function main()
    ...
    if libstart==0 then print("未找到 libil2cpp.so 如果游戏被拆分，则反拆分.") end
    metadata = get_metadata()
    if #metadata==0 then return print("未找到元数据") end
    ...
    "Dump字段", 
    "Dump方法", 
    "深度搜索（慢)"}, {"1000"}, {"number", "checkbox", "checkbox", "checkbox"})
    ...
    for i, v in ipairs(originalResults) do --循环检查搜索列表中的每个地址
        local found = false
        local fixedPointer = fixAddressForPointer(v.address, pointerSize)
        print("当前地址: 0x"..tohex(v.address))
        
        local addrs = {} --
        for off=0, off_range, pointerSize do --循环获取地址值以检查类父指针
        ...
        for i_, v_ in ipairs(parentPtr) do
            ...
            if deepSearch==true or (namespacePtr[i_].value>metadata[1].start and namespacePtr[i_].value<metadata[1]["end"]) then
                ...
                if tmp_class_name~="" then
                    ...
                    print("字段偏移: 0x"..tohex(v.address - addrs[i_].address))
